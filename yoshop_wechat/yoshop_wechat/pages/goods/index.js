...
Page({
  ...
  onLoad: function(options) {
    if(options.member_id){
      wx.setStorageSync('inviter_id', options.member_id);
    }
    let _this = this;
    // 商品id
  ...
  onShow: function(){
    if(wx.getStorageSync('userInfo').inviter_id == null){
      if(wx.getStorageSync('uid') && wx.getStorageSync('inviter_id')){
        App._post_form('Apis/bind_send',{member_id:wx.getStorageSync('uid'),inviter_id:wx.getStorageSync('inviter_id')},function(res){
          if(res.code == 0){
  ...
  getGoodsDetail: function() {
    let _this = this;
    App._get('Apis/gift_detail', {
      pgoods_id: _this.data.goods_id
    }, function(result) {
      if (result.code === 0) {
        // 初始化商品详情数据
        ...
        wxParse.wxParse('tourData', 'html', data.pgoods_body, _this);
      } else {
  ...
  submit: function(e) {
    ...
    if (submitType === 'buyNow') {
      ...
      // 加入购物车
      App._post_form('Apis/add_cart', {
        pgoods_id: _this.data.goods_id,
        pgoods_num: _this.data.goods_num,
  ...
  onShareAppMessage: function() {
    ...
    if(wx.getStorageSync('uid') != undefined){
      return {
        title: '转发',
        path: '/pages/goods/index?&member_id='+wx.getStorageSync('uid') + '&goods_id=' + that.data.detail.pgoods_id,
        success: function(res) {}
      }
    }else{
      return {
        title: '转发',
        path: '/pages/goods/index?goods_id=' + that.data.detail.pgoods_id,
        success: function(res) {}
      }
    }
  },
  stroll: function(){
    ...
    wx.navigateTo({
      url: './stroll/stroll?shop_id=' + shop_id,
    })
  },
  spell: function(){
    var _this = this;
    wx.showModal({
      title: '提示',
      content: '7人成团,发起拼团后如果拼团成功团长将直接升为推广商,享受所邀请会员消费流水的0.5%提成',
      success: function(res){
        if(res.confirm){
          wx.navigateTo({
            url: '../flow/checkout?' + App.urlEncode({
  ...
  group: function(e){
    ...
    wx.showModal({
      title: '提示',
      content: '您确定要参与' + e.currentTarget.dataset.member_name + '发起的拼团吗?',
      success: function(res){
        if(res.confirm){
          wx.navigateTo({
            url: '../flow/checkout?' + App.urlEncode({
              ...
              goods_num: 1,
              type:2,
              spell_id: e.currentTarget.dataset.spell_id,
